...
export function SocialPulse({ ratpCount, sytadinCount, communityCount, xCount = 0 }: SocialPulseProps) {
  ...
  const getStatusLabel = (score: number) => {
    if (score > 80) return 'Fluidité'
    if (score > 50) return 'Sous pression'
    return 'Critique'
  }
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 p-5 bg-gradient-to-br from-bg-surface/80 to-bg-base border-b border-bg-border shadow-sm">
      <div className="flex flex-col gap-3 p-4 rounded-2xl bg-bg-elevated/40 border border-bg-border relative overflow-hidden group">
        ...
        <div className="flex items-center gap-2 text-text-muted">
          ...
          <span className="text-[10px] font-bold uppercase tracking-widest">Santé du réseau</span>
        </div>
      ...
      <div className="flex flex-col gap-3 p-4 rounded-2xl bg-bg-elevated/40 border border-bg-border relative overflow-hidden group">
        ...
        <div className="flex items-center gap-2 text-text-muted">
          ...
          <span className="text-[10px] font-bold uppercase tracking-widest">Alertes actives</span>
        </div>
        ...
        <div className="flex items-center gap-3">
          <Badge count={ratpCount} label="Transport" color="bg-[#00A88F]" />
          <Badge count={sytadinCount} label="Voirie" color="bg-[#1DA1F2]" />
          <Badge count={xCount} label="Veille" color="bg-[#000000]" />
      ...
      <div className="flex flex-col gap-3 p-4 rounded-2xl bg-bg-elevated/40 border border-bg-border relative overflow-hidden group">
        ...
        <div className="flex items-center gap-2 text-text-muted">
          ...
          <span className="text-[10px] font-bold uppercase tracking-widest">Tension sociale</span>
        </div>
        <div className="flex items-baseline gap-2">
