    ...
    <div className="rounded-2xl border border-bg-border bg-bg-surface overflow-hidden">
      ...
      <div className="p-4 space-y-4">
        <div className="grid grid-cols-2 gap-2">
          <div className="bg-bg-elevated rounded-lg p-3 space-y-0.5">
            <div className="flex items-center gap-1.5 text-text-muted">
              ...
              <span className="text-[10px]">Axes utiles</span>
            </div>
          ...
          <div className="bg-bg-elevated rounded-lg p-3 space-y-0.5">
            <div className="flex items-center gap-1.5 text-text-muted">
              ...
              <span className="text-[10px]">Étendue</span>
            </div>
            <p className="text-sm font-bold text-text-primary">
            ...
            {Object.entries(stats.frcDistribution)
              ...
              .map(([frc, count]) => (
                <div
                  ...
                  title={`Classe ${frc} — ${stats.frcLabels[frc]} (${count.toLocaleString('fr-FR')})`}
                />
              ))}
          </div>
              ...
              .map(([frc, count]) => (
                <div key={frc} className="flex items-center gap-1">
                  ...
                  <span className="text-[9px] text-text-muted">
                    Classe {frc} · {count.toLocaleString('fr-FR')}
                  </span>
                </div>
              ))}
          </div>
        ...
        <p className="text-[9px] text-text-muted text-center">
          Lecture consolidée · {stats.totalSegments.toLocaleString('fr-FR')} axes utiles
        </p>
      </div>
    </div>
  )
