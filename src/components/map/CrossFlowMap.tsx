...
export const CrossFlowMap = memo(function CrossFlowMap({ debugMode = false }: { debugMode?: boolean }) {
  ...
  const hoveredTransitLineSlug = useMapStore(s => s.hoveredTransitLineSlug)
  const focusedTransitLineSlug = useMapStore(s => s.focusedTransitLineSlug)
  ...
  useEffect(() => {
    ...
    const hoverFilter = hoveredTransitLineSlug
      ? ['==', ['upcase', ['to-string', ['get', 'ref']]], hoveredTransitLineSlug.toUpperCase()]
      : ['==', ['get', 'ref'], '__none__']
    const focusFilter = focusedTransitLineSlug
      ? ['==', ['upcase', ['to-string', ['get', 'ref']]], focusedTransitLineSlug.toUpperCase()]
      : ['==', ['get', 'ref'], '__none__']

    safeSetFilter(map, TRANSIT_ROUTES_SOURCE + '-hover-glow', hoverFilter)
    safeSetFilter(map, TRANSIT_ROUTES_SOURCE + '-hover-line', hoverFilter)
    safeSetFilter(map, TRANSIT_ROUTES_SOURCE + '-focus-glow', focusFilter)
    safeSetFilter(map, TRANSIT_ROUTES_SOURCE + '-focus-line', focusFilter)
  }, [focusedTransitLineSlug, hoveredTransitLineSlug, mapLoaded])

  useEffect(() => {
    const map = mapRef.current
    if (!mapLoaded || !map || !focusedTransitLineSlug) return

    const source = map.getSource(TRANSIT_ROUTES_SOURCE) as (maplibregl.GeoJSONSource & {
    ...
    const matches = featureCollection.features.filter((feature) => {
      ...
      return String(feature.properties?.ref ?? '').toUpperCase() === focusedTransitLineSlug.toUpperCase()
    })
  ...
  }, [focusedTransitLineSlug, mapLoaded])
  ...
  useEffect(() => {
    ...
    safeSetLayoutProperty(map, TRANSIT_ROUTES_SOURCE + '-hover-glow', 'visibility', transportVis)
    safeSetLayoutProperty(map, TRANSIT_ROUTES_SOURCE + '-hover-line', 'visibility', transportVis)
    safeSetLayoutProperty(map, TRANSIT_ROUTES_SOURCE + '-focus-glow', 'visibility', transportVis)
    safeSetLayoutProperty(map, TRANSIT_ROUTES_SOURCE + '-focus-line', 'visibility', transportVis)
    ...
    if (isDecisionMap) {
      ...
      safeSetLayoutProperty(map, TRANSIT_ROUTES_SOURCE + '-hover-glow', 'visibility', 'none')
      safeSetLayoutProperty(map, TRANSIT_ROUTES_SOURCE + '-hover-line', 'visibility', 'none')
      safeSetLayoutProperty(map, TRANSIT_ROUTES_SOURCE + '-focus-glow', 'visibility', 'none')
      safeSetLayoutProperty(map, TRANSIT_ROUTES_SOURCE + '-focus-line', 'visibility', 'none')
...
function initStaticSources(map: maplibregl.Map) {
  ...
  map.addLayer({
    id:      TRANSIT_ROUTES_SOURCE + '-hover-glow',
    type:    'line',
    source:  TRANSIT_ROUTES_SOURCE,
    minzoom: 10,
    filter:  ['==', ['get', 'ref'], '__none__'],
    layout:  { 'line-join': 'round', 'line-cap': 'round' },
    paint:   {
      'line-color': '#FFFFFF',
      'line-width': ['interpolate', ['linear'], ['zoom'], 10, 6, 13, 10, 16, 13],
      'line-opacity': 0.26,
      'line-blur': 0.6,
    },
  })
  ...
  map.addLayer({
    id:      TRANSIT_ROUTES_SOURCE + '-hover-line',
    type:    'line',
    source:  TRANSIT_ROUTES_SOURCE,
    minzoom: 10,
    filter:  ['==', ['get', 'ref'], '__none__'],
    ...
    paint:   {
      'line-color': ['get', 'color'],
      'line-width': ['interpolate', ['linear'], ['zoom'], 10, 4, 13, 7, 16, 9],
      'line-opacity': 0.96,
    },
  })

  map.addLayer({
    id:      TRANSIT_ROUTES_SOURCE + '-focus-glow',
    type:    'line',
    source:  TRANSIT_ROUTES_SOURCE,
    minzoom: 10,
    filter:  ['==', ['get', 'ref'], '__none__'],
    ...
    paint:   {
      'line-color': '#0F172A',
      'line-width': ['interpolate', ['linear'], ['zoom'], 10, 8, 13, 12, 16, 16],
      'line-opacity': 0.18,
      'line-blur': 0.5,
    },
  })
  ...
  map.addLayer({
    id:      TRANSIT_ROUTES_SOURCE + '-focus-line',
    type:    'line',
    source:  TRANSIT_ROUTES_SOURCE,
    minzoom: 10,
    filter:  ['==', ['get', 'ref'], '__none__'],
    ...
    paint:   {
      'line-color': ['get', 'color'],
      'line-width': ['interpolate', ['linear'], ['zoom'], 10, 5, 13, 8, 16, 10],
      'line-opacity': 1,
    },
  })
