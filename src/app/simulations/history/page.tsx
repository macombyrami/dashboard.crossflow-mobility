...
export default function SimulationHistoryPage() {
  ...
  return (
    <div className="flex flex-col h-full bg-[#0A0B0E] text-white">
      {/* Header Area */}
      <div className="p-4 sm:p-6 border-b border-white/5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-[#111218]/50 backdrop-blur-xl sticky top-0 z-10">
        <div className="flex items-center gap-3 sm:gap-4">
          <Link 
            href="/simulation"
            className="p-1.5 sm:p-2 rounded-xl bg-white/5 border border-white/10 hover:bg-white/10 transition-all group shrink-0"
          >
            <ArrowLeft className="w-4 h-4 sm:w-5 sm:h-5 group-hover:-translate-x-1 transition-transform" />
          </Link>
          <div className="min-w-0">
            <h1 className="text-base sm:text-lg lg:text-xl font-bold tracking-tight uppercase italic flex items-center gap-2 truncate">
              <History className="w-4 h-4 sm:w-5 sm:h-5 text-brand shrink-0" />
              Historique
            </h1>
            <p className="text-[10px] text-text-muted mt-0.5 uppercase tracking-widest opacity-60 truncate">
              Gérez vos scénarios passés
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 sm:gap-3 w-full sm:w-auto">
          <div className="relative group flex-1 sm:flex-initial">
            ...
            <input 
              ...
              className="pl-9 pr-4 py-2 bg-white/5 border border-white/5 rounded-xl text-sm focus:outline-none focus:border-brand/40 focus:bg-white/10 transition-all w-full sm:w-48 lg:w-64"
            />
          </div>
          <button 
...
function SimulationCard({ result }: { result: any }) {
  ...
  return (
    <div className="group bg-[#141519] border border-white/5 rounded-2xl p-4 sm:p-5 hover:border-brand/30 transition-all hover:translate-y-[-2px] shadow-lg flex flex-col h-full">
