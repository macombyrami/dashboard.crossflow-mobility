...
export default function ConsultantAIPage() {
  ...
  const memoizedContext = useMemo(() => {
    return {
      ...
      events: ['Marathon de Paris 2026 (passé)', 'PSG vs Rennes (match de ce soir)'],
      social: ['Signaux de ralentissement sur le périphérique', 'Plusieurs messages mentionnent des perturbations sur la ligne 13'],
    ...
    <main className="min-h-full p-4 sm:p-6 lg:p-8 flex flex-col gap-6 lg:gap-8 animate-in fade-in duration-1000 pb-safe">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-6 border-b border-white/5">
        ...
        <div className="flex flex-wrap items-center gap-2 sm:gap-3">
           <StatusBadge icon={<ShieldCheck className="w-3.5 h-3.5" />} label="Mode décisionnel" color="text-emerald-400 bg-emerald-400/5 border-emerald-400/10" />
           <StatusBadge icon={<Database className="w-3.5 h-3.5" />} label="Données actives" color="text-brand bg-brand/5 border-brand/10" />
           <StatusBadge icon={<Globe className="w-3.5 h-3.5" />} label="Vue consolidée" color="text-blue-400 bg-blue-400/5 border-blue-400/10" />
        </div>
      </div>
        ...
        <div className="hidden xl:flex flex-col gap-6">
           <div className="glass-card p-6 rounded-[24px] border border-white/5 space-y-4">
              <div className="flex items-center gap-2 mb-2">
                ...
                <h3 className="text-[11px] font-bold text-white uppercase tracking-[0.2em]">Lecture contextuelle</h3>
              </div>
              <FeedItem label="Zone suivie" value={city.name} />
              <FeedItem label="Niveau de fiabilité" value="94.2%" color="text-brand" />
              <FeedItem label="Points d’attention sociaux" value={memoizedContext.social.length.toString()} />
              
              <div className="pt-4 border-t border-white/5 mt-4">
                <p className="text-[12px] text-text-muted leading-relaxed">
                  Le moteur d’analyse consolide actuellement <strong>1 240 signaux</strong> par seconde pour générer vos diagnostics.
                </p>
              </div>
           </div>
           ...
           <div className="p-1 rounded-[24px] bg-gradient-to-br from-brand/20 to-transparent">
             <div className="glass-card p-6 rounded-[23px] bg-[#0A0B0E]/90">
                <h3 className="text-[11px] font-bold text-text-muted uppercase tracking-[0.2em] mb-4">Lecture premium</h3>
                <p className="text-[13px] text-white/90 leading-snug font-medium mb-6">
                  Besoin d’un rapport détaillé pour un comité ou une réunion de pilotage ?
                </p>
                <button className="w-full py-2.5 rounded-xl bg-white/5 border border-white/10 text-[12px] font-bold text-white hover:bg-white/10 transition-all flex items-center justify-center gap-2">
                  Générer un rapport de pilotage
